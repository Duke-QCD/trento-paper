...
collision.  By attenuation, we mean \textbf{WHAT?}, as shown in FIG.~\ref{fig:saturation}.  The model behaves
quite differently depending on $p$, for instance $p=1$ is precisely a wounded nucleon model, while
$-1 \lesssim p \lesssim 0$ mimics CGC saturation.
...
\begin{itemize}
  \item continuous;
  \item monotonic, hence if
    $T_A \leq T_A'$, $T_B \leq T_B'$, then
    $T_R(p; T_A, T_B) \leq T_R(p; T_A', T_B')$;
  \item and symmetric:
    $T_R(p; T_A, T_B) = T_R(p; T_B, T_A)$.
    Indeed, the reduced thickness is independent of $p$ when $T_A$ equals $T_B$: $T_R(p; T, T) = T$.
  \item Finally, it vanishes when both $T_A$ and $T_B$ vanish: $T_R(p; 0, 0) = 0$.
    ...
    positive values of $p$ provide a physically meaningful model, since it violates the principle of eikonal
    entropy production.
\end{itemize}
