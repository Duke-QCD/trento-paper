...
Therefore $N$ one-on-one nucleon collisions should produce the same amount of entropy as a single $N$-on-$N$ collision, which is mathematically equivalent to the function $f$ being scale-invariant:
\begin{equation}
...
Consequently, \trento\ predicts roughly the same number of charged particles multiplicity in tip-tip and side-side uranium-uranium collisions.
