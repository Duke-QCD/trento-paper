...
\begin{itemize*}
  ...
  \item The reduced thickness vanishes when both $T_A$ and $T_B$ vanish: $T_R(p; 0, 0) = 0$.  In fact, for $p
    \leq 0$, $T_R$ vanishes if \emph{either} $T_A$ or $T_B$ do.  It is unclear whether positive values of $p$
    yield a physically meaningful model, since it violates the postulate of eikonal entropy production.
\end{itemize*}
