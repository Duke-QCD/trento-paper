...
However, recent results from ultra-central uranium-uranium collisions at RHIC \cite{FortheSTAR:2013bza,Wang:2014qxa} show that particle production does not scale with the number of nucleon-nucleon collisions, excluding even a small contribution to entropy production from binary collisions \cite{Goldschmidt:2015qya}.
We therefore additionally postulate that the function $f$ must be scale-invariant:
\begin{equation}
  f(c \, T_A, c \, T_B) = c \, f(T_A, T_B)
  \label{eq:scale-inv}
\end{equation}
for any nonzero constant $c$.
The binary collision term $(\alpha \, T_A T_B)$ clearly does not satisfy this constraint.
On the other hand, a new participant quark model was able to fit the transverse-energy distributions of proton-proton, deuteron-gold, and gold-gold collisions at RHIC without invoking binary collision scaling \cite{PhysRevC.89.044905}.
This suggests that binary collisions are not necessary to describe the data, and in fact may only be a proxy for a different physical process.
...
as $p$ decreases, asymmetric regions of the collision produce less entropy, which accentuates the elliptical overlap shape in non-central collisions and enhances their eccentricity.
...
In summary, we have developed \trento, a new effective initial condition model for high-energy nuclear collisions based on eikonal entropy deposition via a ``reduced thickness'' function.
...
\trento\ introduces a new ansatz for entropy production, distinct from the two-component wounded nucleon and binary collision ansatz.
It smoothly interpolates entropy deposition models ranging from a traditional wounded nucleon prescription to models that only deposit entropy in regions of nucleon-nucleon overlap.
A preliminary comparison of multiplicity distributions and eccentricity harmonics to LHC data prefers an overlap-type mechanism.
