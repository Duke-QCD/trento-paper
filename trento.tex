...
Suppose a pair of projectiles labeled $A$, $B$ collide along beam axis $z$.
Each projectile is represented by its nuclear thickness function including only \emph{participant} matter:
\begin{equation}
...
The shape parameter $k$ is tuned to optimally fit the data:
...
Each $p$ value also yields a reasonable fit to the shapes of the proton-lead and lead-lead distributions, although $p = 0$ appears to be  favored for lead-lead.
...
This may be understood by the attenuation property of the reduced thickness:
