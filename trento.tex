...
\end{equation}
where $N_{coll,i}$ denotes the number of collisions suffered by the $i$-th participant.

In figure [\ref{fig:cdepend}] we show the centrality dependence of charged particle production calculated from the generalized mean and two-component models for different 
values of the exponent $p$ and binary collision fraction $\alpha$. 
%\begin{figure}[hb]
% \includegraphics[width=\linewidth]{./fig/centrality-depend.pdf}
% % centrality-depend.pdf: 0x0 pixel, 0dpi, nanxnan cm, bb=
% \caption{\label{fig:cdepend} Centrality dependence charged particle production for the two-component model and generalized mean compared to ALICE PbPb data at 2.76 TeV
% for $B=0.36\, fm^2$, $k=0.8$ $\alpha = (0.0,0.30), \delta\alpha=0.05$ and $p=(-1,1), \delta p=0.5$.}
%\end{figure}
We see that binary collision scaling and projectile-target attentuation provide two different explanations for the centrality dependence of charged particle production.

To differentiate between these two mechanisms, it makes sense to examine a collision system for which the predictions of attentuation and binary collision scaling are 
maximally different.

\section{Signals of binary collision breakdown in UU collisions}
The prolate deformation of the Uranium nucleus admits two distinct configurations of Uranium collisions in which the density of nuclear matter is fully overlapping 
- tip-to-tip collisions where the Uranium symmetry axis is alligned with the beam axis and side-on-side collisions where the Uranium 
symmetry axis is perpendicular to the beam axis. 
\begin{figure}
 \centering
