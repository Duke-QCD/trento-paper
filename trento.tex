...
\begin{equation}
  P_\text{coll} = 1 - \exp\biggl[ -\sigma_{p} \int dx \, dy \int dz \, \rho_A \int dz \, \rho_B \biggr],
  \label{eq:pcoll}
\end{equation}
where the integral in the exponential is the overlap integral of the proton thickness functions and
$\sigma_{p}$ is a cross-section which is tuned such that the total proton-proton cross-section in our model equals the
experimentally measured inelastic nucleon nucleon cross-section $\sigma_{NN}$.
...
The reduced thickness is then calculated from the projectile thickness function; this furnishes the initial
...
projectile $A$, $B$, utilizing either a uncorrelated Woods-Saxon distribution or correlated nuclear configurations when available \cite{Alvioli:2009ab}.
Subsequently the collision probability \eqref{eq:pcoll} is sampled for each pairwise interaction.
Those nucleons that collide with at least one partner are labeled ``participants'' and the rest are discarded.
The fluctuated thickness function for the projectile $A$ reads then
\begin{equation}
...
\textbf{SOMETHING HERE? How about a brief recap on what the parameters of the model are and what the
model produces as output, i.e. energy-density/entropy deposition as init. cond. for Hydro...}
